...
namespace espressopp {
  ...
  namespace integrator {
    ...
    VelocityVerletLE::~VelocityVerletLE()
    {
      LOG4ESPP_INFO(theLogger, "free VelocityVerletLE");
    }
...
//std::cout<<" aftIntP> "<<" \n";}
        ...
        int ctmp=static_cast<int>(floor(shearRate*getStep()*getTimeStep()*ngrid*Lz/Lx+0.5));
        int cshift=static_cast<int>(floor(shearRate*(getStep()+1.0)*getTimeStep()*ngrid*Lz/Lx+0.5));
//system.shearOffset=shearRate*Lz*(getStep()+1.0)*getTimeStep();
        if (cshift>ctmp) {
//Particle *p2;
//if (rename("FLAG_P","FLAG_P")==0){
//p2=storage.lookupLocalParticle(6627);
//std::cout<<" BEF-"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") \n";
//p2=storage.lookupRealParticle(6627);
//if (p2) std::cout<<"    -"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") \n";
//}
//system.comm->barrier();

          shift_count++;storage.remapNeighbourCells(shift_count);
          system.ghostShift=shift_count;
          resortFlag = true;
//if (rename("FLAG_P","FLAG_P")==0){
//p2=storage.lookupLocalParticle(6627);
//std::cout<<" AFT-"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") \n";
//p2=storage.lookupRealParticle(6627);
//if (p2) std::cout<<"    -"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") \n";
//}
//system.comm->barrier();
...
//if (rename("FLAG_P","FLAG_P")==0){
//p2=storage.lookupLocalParticle(6627);
//if (p2) std::cout<<"RES-Look-"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") "<<" \n";
//}
//system.comm->barrier();
            storage.decompose();
//if (rename("FLAG_P","FLAG_P")==0){
//p2=storage.lookupLocalParticle(6627);
//if (p2) std::cout<<" rrr-"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") "<<" \n";
//}
//system.comm->barrier();
            maxDist  = 0.0;
            resortFlag = false;
...
//std::cout<<" aftIntV> "<<" \n";}
    ...
    int VelocityVerletLE::getNumResorts() const
    {
      return nResorts;
    }
...
//Need to add propagation of shear speed if necessary
    ...
    void VelocityVerletLE::integrate2_extra()
    {
      ...
      for(CellListIterator cit(realCells); !cit.isDone(); ++cit) {
        ...
        if (itmp<0) itmp=0;
        else if (itmp>=bin_size) itmp=bin_size-1;
...
//System& system = getSystemRef();
//Particle *p2;
//if (rename("FLAG_P","FLAG_P")==0 && system.comm->rank()%4==1){
//p2=storage.lookupRealParticle(6626);
//if (p2){
//p2=storage.lookupLocalParticle(6627);
//std::cout<<"UPD-Look-"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") \n";
//}}
      { 
        VT_TRACER("commF");
...
//if (p2){
//p2=storage.lookupLocalParticle(6627);
//std::cout<<" uuu-"<<system.comm->rank()<<"> "<<p2->id()<<" ("<<p2->position()<<") \n";
//}}
      {
        VT_TRACER("commR");
